...
export default async function EditAccountPage({ searchParams }: EditAccountPageProps) {
  ...
  const { data: filmLinks } = playerProfileId
    ? await supabase.from("film_links").select("id, url, provider, film_type, label, is_default").eq("player_profile_id", playerProfileId).order("created_at", { ascending: false }).returns<FilmLink[]>()
    : { data: [] as FilmLink[] };
