...
interface DailyWebhookEvent {
  type?: string;
  id?: string;
  payload?: { room?: string; start_ts?: number; end_ts?: number };
  test?: string;
}
...
function validSignature(body: string, timestamp: string | null, signature: string | null) {
  ...
  try {
    const expected = createHmac("sha256", Buffer.from(secret, "base64")).update(`${timestamp}.${body}`).digest();
...
export async function POST(request: Request) {
  ...
  // Daily verifies a webhook URL with this connectivity probe before activation.
  ...
  const completedAt = event.payload.end_ts ? new Date(event.payload.end_ts * 1000).toISOString() : new Date().toISOString();
