...
export default async function SignInPage({ searchParams }: SignInPageProps) {
  ...
  return (
    <AuthShell eyebrow="Welcome Back" title="Sign in and keep scouting.">
      {notice ? <p className="mb-4 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-3 text-sm font-bold text-emerald-200 dark:border-emerald-400/30 dark:bg-emerald-500/10 dark:text-emerald-200">{notice}</p> : null}
      {error ? <p className="mb-4 rounded-2xl border border-red-400/30 bg-red-500/10 p-3 text-sm font-bold text-red-200 dark:border-red-400/30 dark:bg-red-500/10 dark:text-red-200">{error}</p> : null}
      <form action={signInAction} className="space-y-4">
        <input type="hidden" name="next" value={next ?? "/dashboard"} />
        <label className="block">
          <span className="text-sm font-black uppercase tracking-wide text-slate-300 dark:text-slate-300">Email</span>
          <input name="email" type="email" required autoComplete="email" className="mt-2 h-12 w-full rounded-2xl border border-slate-200 bg-white/85 px-4 text-sm font-semibold text-slate-900 outline-none backdrop-blur-xl transition focus:border-red-400 focus:ring-4 focus:ring-red-100 dark:border-white/10 dark:bg-white/10 dark:text-white dark:focus:ring-red-500/20" />
        </label>
        <label className="block">
          <div className="flex items-center justify-between">
            <span className="text-sm font-black uppercase tracking-wide text-slate-300 dark:text-slate-300">Password</span>
