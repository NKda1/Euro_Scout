...
export default async function Navbar() {
  ...
  const notificationSummary = profile ? await getNotificationSummary(profile, user?.email) : null;
  const notificationCount = notificationSummary?.total ?? 0;
    ...
    <nav className="sticky top-0 z-40 border-b border-slate-200 bg-white dark:border-white/10 dark:bg-[#090909]">
      <div className="mx-auto flex h-[72px] max-w-[112rem] items-center justify-between gap-5 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex shrink-0 items-center gap-3">
          {user ? (
            <>
              {notificationSummary ? <NotificationBell summary={notificationSummary} /> : null}
              <Link
                href={routes.account}
